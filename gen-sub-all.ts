...
function genSubscriberOutboxTriggerSql(name: string, a: TableAnalysis, dbSchema: string): string {
  if (!a.hasRealtimeSubscription) return ''

  return `
-- Why: enqueue row change events to subscriber_outbox for service-role worker processing.
CREATE OR REPLACE FUNCTION ${dbSchema}.enqueue_${name}_subscriber_outbox()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    PERFORM ${dbSchema}.enqueue_subscriber_outbox(
      '${name}',
      TG_OP,
  ...
  IF TG_OP = 'DELETE' THEN
    PERFORM ${dbSchema}.enqueue_subscriber_outbox(
      '${name}',
      TG_OP,
      jsonb_build_object('${a.primary.name}', OLD.${a.primary.name}),
      to_jsonb(OLD),
      NULL
    );
    RETURN OLD;
...
DROP TRIGGER IF EXISTS ${name}_subscriber_outbox_enqueue ON ${dbSchema}.${name};
CREATE TRIGGER ${name}_subscriber_outbox_enqueue
  AFTER INSERT OR UPDATE OR DELETE ON ${dbSchema}.${name}
  FOR EACH ROW EXECUTE FUNCTION ${dbSchema}.enqueue_${name}_subscriber_outbox();`
}
...
function genSubscriberOutboxInfra(dbSchema: string): string {
  const createSchema = dbSchema === 'public' ? '' : `CREATE SCHEMA IF NOT EXISTS ${dbSchema};\n\n`
  return `-- ═══════════════════════════════════════════════════════════════
-- Subscriber Outbox Infrastructure
-- Generated by code-gen v2
-- ═══════════════════════════════════════════════════════════════

${createSchema}
CREATE TABLE IF NOT EXISTS ${dbSchema}.subscriber_outbox (
  id bigserial PRIMARY KEY,
  topic text NOT NULL,
...
-- Why: make claiming ready outbox messages efficient for worker polling.
CREATE INDEX IF NOT EXISTS subscriber_outbox_ready_idx
  ON ${dbSchema}.subscriber_outbox (available_at, id)
  WHERE status IN ('ready', 'processing');

-- Why: filter outbox jobs by topic when workers are scoped per table/topic.
CREATE INDEX IF NOT EXISTS subscriber_outbox_topic_idx
  ON ${dbSchema}.subscriber_outbox (topic, available_at, id)
  WHERE status IN ('ready', 'processing');

-- Why: provide one insertion point for table triggers.
CREATE OR REPLACE FUNCTION ${dbSchema}.enqueue_subscriber_outbox(
  p_topic text,
  p_event_type text,
  p_row_pk jsonb,
...
LANGUAGE sql
SECURITY DEFINER
SET search_path = ${dbSchema}, public
AS $$
  INSERT INTO ${dbSchema}.subscriber_outbox (
    topic, event_type, row_pk, old_row, new_row, available_at, max_attempts
  )
  VALUES (
    p_topic, upper(p_event_type), p_row_pk, p_old_row, p_new_row, p_available_at, GREATEST(p_max_attempts, 1)
  )
  RETURNING id;
$$;

-- Why: atomically claim a batch of ready/timed-out messages using row locks.
CREATE OR REPLACE FUNCTION ${dbSchema}.claim_subscriber_outbox(
  p_worker_id text,
  p_limit int DEFAULT 10,
  p_visibility_timeout_seconds int DEFAULT 60,
  p_topic text DEFAULT NULL
)
RETURNS SETOF ${dbSchema}.subscriber_outbox
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ${dbSchema}, public
AS $$
BEGIN
  ...
  WITH next_batch AS (
    SELECT id
    FROM ${dbSchema}.subscriber_outbox
    WHERE
      attempts < max_attempts
      AND available_at <= now()
      AND (status = 'ready' OR status = 'processing')
      AND (p_topic IS NULL OR topic = p_topic)
    ORDER BY id
    FOR UPDATE SKIP LOCKED
    LIMIT GREATEST(p_limit, 1)
  )
  UPDATE ${dbSchema}.subscriber_outbox q
  SET
    status = 'processing',
    locked_at = now(),
    locked_by = p_worker_id,
    attempts = q.attempts + 1,
    available_at = now() + make_interval(secs => GREATEST(p_visibility_timeout_seconds, 1)),
    updated_at = now()
  WHERE q.id IN (SELECT id FROM next_batch)
  RETURNING q.*;
END;
$$;

-- Why: acknowledge successful processing by the current worker.
CREATE OR REPLACE FUNCTION ${dbSchema}.ack_subscriber_outbox(
  p_id bigint,
  p_worker_id text
)
RETURNS void
...
SET search_path = ${dbSchema}, public
AS $$
  UPDATE ${dbSchema}.subscriber_outbox
  SET
    status = 'done',
    locked_at = NULL,
    locked_by = NULL,
...
-- Why: release failed processing back to ready queue with retry backoff.
CREATE OR REPLACE FUNCTION ${dbSchema}.nack_subscriber_outbox(
  p_id bigint,
  p_worker_id text,
...
-- Why: allow table writers to enqueue events via trigger-driven function calls.
GRANT EXECUTE ON FUNCTION ${dbSchema}.enqueue_subscriber_outbox(
  text, text, jsonb, jsonb, jsonb, timestamptz, int
) TO anon, authenticated, service_role;

-- Why: restrict worker dequeue/ack/nack controls to backend service-role only.
REVOKE ALL ON FUNCTION ${dbSchema}.claim_subscriber_outbox(text, int, int, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION ${dbSchema}.ack_subscriber_outbox(bigint, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION ${dbSchema}.nack_subscriber_outbox(bigint, text, int, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION ${dbSchema}.claim_subscriber_outbox(text, int, int, text) TO service_role;
GRANT EXECUTE ON FUNCTION ${dbSchema}.ack_subscriber_outbox(bigint, text) TO service_role;
GRANT EXECUTE ON FUNCTION ${dbSchema}.nack_subscriber_outbox(bigint, text, int, text) TO service_role;
`
}
...
function genDrops(name: string, table: TableSchema, a: TableAnalysis, dbSchema: string): string {
  ...
  if (a.hasRealtimeSubscription) {
    drops.push(`DROP TRIGGER IF EXISTS ${name}_subscriber_outbox_enqueue ON ${dbSchema}.${name} CASCADE;`)
    drops.push(`DROP FUNCTION IF EXISTS ${dbSchema}.enqueue_${name}_subscriber_outbox CASCADE;`)
  }
...
function generateSQL(name: string, table: TableSchema, a: TableAnalysis, knownTables: string[] = [], dbSchema = 'public'): string {
  ...
  const subscriberOutboxTriggerSql = genSubscriberOutboxTriggerSql(name, a, dbSchema)
  if (subscriberOutboxTriggerSql) sections.push(subscriberOutboxTriggerSql)
...
function generateSubscriberWorkerFile(
  tables: Array<{ name: string; analysis: TableAnalysis }>,
  defaultDbSchema = 'public'
): string {
  const lines: string[] = []
  ...
  lines.push(`const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY ?? "";`)
  lines.push(`const SUPABASE_DB_SCHEMA = process.env.SUPABASE_DB_SCHEMA ?? "${defaultDbSchema}";`)
  lines.push(`const TARGET_TOPIC = process.env.TARGET_TABLE ?? process.env.TARGET_TOPIC ?? "";`)
  lines.push(`const WORKER_ID = process.env.WORKER_ID ?? \`subscriber-worker-\${process.pid}-\${randomUUID().slice(0, 8)}\`;`)
  lines.push(`const POLL_INTERVAL_MS = Number(process.env.OUTBOX_POLL_INTERVAL_MS ?? "1000");`)
  lines.push(`const CLAIM_BATCH_SIZE = Number(process.env.OUTBOX_CLAIM_BATCH_SIZE ?? "20");`)
  lines.push(`const VISIBILITY_TIMEOUT_SECONDS = Number(process.env.OUTBOX_VISIBILITY_TIMEOUT_SECONDS ?? "120");`)
  lines.push(`const RETRY_BASE_BACKOFF_SECONDS = Number(process.env.OUTBOX_RETRY_BASE_BACKOFF_SECONDS ?? "3");`)
  lines.push(`const RETRY_MAX_BACKOFF_SECONDS = Number(process.env.OUTBOX_RETRY_MAX_BACKOFF_SECONDS ?? "300");`)
  lines.push('')
  lines.push(`const ELIGIBLE_TOPICS = [${tables.map(t => `"${t.name}"`).join(', ')}] as const;`)
  lines.push('')
  lines.push(`type WorkerClient = any;`)
  lines.push('')
  lines.push(`type OutboxRow = {`)
  lines.push(`  id: number;`)
  lines.push(`  topic: string;`)
  lines.push(`  event_type: "INSERT" | "UPDATE" | "DELETE";`)
  lines.push(`  row_pk: Record<string, any>;`)
  lines.push(`  old_row: Record<string, any> | null;`)
  lines.push(`  new_row: Record<string, any> | null;`)
  lines.push(`  attempts: number;`)
  lines.push(`  max_attempts: number;`)
  lines.push(`};`)
  lines.push('')
  lines.push(`const sleep = (ms: number): Promise<void> => new Promise(resolve => setTimeout(resolve, ms));`)
  lines.push('')
  lines.push(`function computeBackoffSeconds(attempts: number): number {`)
  lines.push(`  const attempt = Math.max(attempts, 1);`)
  lines.push(`  const exp = Math.min(attempt, 10);`)
  lines.push(`  const delay = RETRY_BASE_BACKOFF_SECONDS * (2 ** (exp - 1));`)
  lines.push(`  return Math.min(Math.max(1, delay), RETRY_MAX_BACKOFF_SECONDS);`)
  lines.push(`}`)
  lines.push('')
  lines.push(`async function claimBatch(client: WorkerClient): Promise<OutboxRow[]> {`)
  lines.push(`  const topic = TARGET_TOPIC || null;`)
  lines.push(`  const { data, error } = await client.rpc("claim_subscriber_outbox", {`)
  lines.push(`    p_worker_id: WORKER_ID,`)
  lines.push(`    p_limit: CLAIM_BATCH_SIZE,`)
  lines.push(`    p_visibility_timeout_seconds: VISIBILITY_TIMEOUT_SECONDS,`)
  lines.push(`    p_topic: topic`)
  lines.push(`  });`)
  lines.push(`  if (error) throw new Error(\`Failed to claim outbox rows: \${error.message}\`);`)
  lines.push(`  return (data ?? []) as OutboxRow[];`)
  lines.push(`}`)
  lines.push('')
  lines.push(`async function ack(client: WorkerClient, rowId: number): Promise<void> {`)
  lines.push(`  const { error } = await client.rpc("ack_subscriber_outbox", { p_id: rowId, p_worker_id: WORKER_ID });`)
  lines.push(`  if (error) throw new Error(\`Failed to ack outbox row \${rowId}: \${error.message}\`);`)
  lines.push(`}`)
  lines.push('')
  lines.push(`async function nack(client: WorkerClient, row: OutboxRow, errorMessage: string): Promise<void> {`)
  lines.push(`  const backoff = computeBackoffSeconds(row.attempts);`)
  lines.push(`  const { error } = await client.rpc("nack_subscriber_outbox", {`)
  lines.push(`    p_id: row.id,`)
  lines.push(`    p_worker_id: WORKER_ID,`)
  lines.push(`    p_backoff_seconds: backoff,`)
  lines.push(`    p_error: errorMessage`)
  lines.push(`  });`)
  lines.push(`  if (error) throw new Error(\`Failed to nack outbox row \${row.id}: \${error.message}\`);`)
  lines.push(`}`)
  lines.push('')
  lines.push(`async function processOutboxRow(row: OutboxRow): Promise<void> {`)
  lines.push(`  const pk = JSON.stringify(row.row_pk ?? {});`)
  lines.push(`  console.log(\`[worker] topic=\${row.topic} event=\${row.event_type} row=\${pk} attempt=\${row.attempts}/\${row.max_attempts}\`);`)
  lines.push(`  // TODO: implement business logic here (fanout, webhooks, downstream jobs, etc).`)
  lines.push(`}`)
  lines.push('')
  lines.push(`async function main(): Promise<void> {`)
  ...
  lines.push(`  });`)
  lines.push(``)
  lines.push(`  console.log("[worker] started");`)
  lines.push(`  console.log("[worker] schema:", SUPABASE_DB_SCHEMA);`)
  lines.push(`  console.log("[worker] worker_id:", WORKER_ID);`)
  lines.push(`  console.log("[worker] topics:", TARGET_TOPIC || ELIGIBLE_TOPICS.join(", "));`)
  lines.push(``)
  lines.push(`  let isRunning = true;`)
  lines.push(`  const shutdown = () => {`)
  lines.push(`    isRunning = false;`)
  lines.push(`    console.log("[worker] shutdown signal received");`)
  lines.push(`  };`)
  lines.push(`  process.on("SIGINT", shutdown);`)
  lines.push(`  process.on("SIGTERM", shutdown);`)
  lines.push(``)
  lines.push(`  while (isRunning) {`)
  lines.push(`    let claimed: OutboxRow[] = [];`)
  lines.push(`    try {`)
  lines.push(`      claimed = await claimBatch(client);`)
  lines.push(`    } catch (error) {`)
  lines.push(`      console.error("[worker] claim failed:", error);`)
  lines.push(`      await sleep(POLL_INTERVAL_MS);`)
  lines.push(`      continue;`)
  lines.push(`    }`)
  lines.push(``)
  lines.push(`    if (!claimed.length) {`)
  lines.push(`      await sleep(POLL_INTERVAL_MS);`)
  lines.push(`      continue;`)
  lines.push(`    }`)
  lines.push(``)
  lines.push(`    for (const row of claimed) {`)
  lines.push(`      try {`)
  lines.push(`        await processOutboxRow(row);`)
  lines.push(`        await ack(client, row.id);`)
  lines.push(`      } catch (error) {`)
  lines.push(`        const message = error instanceof Error ? error.message : String(error);`)
  lines.push(`        console.error(\`[worker] row \${row.id} failed:\`, message);`)
  lines.push(`        try {`)
  lines.push(`          await nack(client, row, message);`)
  lines.push(`        } catch (nackError) {`)
  lines.push(`          console.error(\`[worker] row \${row.id} nack failed:\`, nackError);`)
  lines.push(`        }`)
  lines.push(`      }`)
  lines.push(`    }`)
  lines.push(`  }`)
  lines.push(``)
  lines.push(`  console.log("[worker] stopped");`)
  lines.push(`}`)
  lines.push('')
  lines.push(`main().catch((error) => {`)
  lines.push(`  console.error(error);`)
...
async function generate(schemaPath: string, outputDir: string): Promise<void> {
  ...
  if (realtimeEligibleTables.length) {
    const subscriberWorker = generateSubscriberWorkerFile(realtimeEligibleTables, dbSchema)
    await writeFile(path.join(dir, 'subscriber.ts'), subscriberWorker)
    console.log(`  ✓ subscriber.ts`)
    await writeFile(path.join(dir, 'sql/subscriber_outbox.sql'), genSubscriberOutboxInfra(dbSchema))
    console.log(`  ✓ sql/subscriber_outbox.sql`)
  } else {
    const subscriberPath = path.join(dir, 'subscriber.ts')
    if (fs.existsSync(subscriberPath)) {
      await fs.promises.unlink(subscriberPath)
    }
    const subscriberOutboxSqlPath = path.join(dir, 'sql/subscriber_outbox.sql')
    if (fs.existsSync(subscriberOutboxSqlPath)) {
      await fs.promises.unlink(subscriberOutboxSqlPath)
    }
  }

  const legacySubscriberExamplePath = path.join(dir, 'subscriber.example.ts')
  if (fs.existsSync(legacySubscriberExamplePath)) {
    await fs.promises.unlink(legacySubscriberExamplePath)
  }
  const legacyInvokeChangePath = path.join(dir, 'invoke_change.example.ts')
  if (fs.existsSync(legacyInvokeChangePath)) {
    await fs.promises.unlink(legacyInvokeChangePath)
  }
